...
Loss function: reverse KL#
On-policy distillation can use a variety of loss functions for grading the student’s trajectories.See Agarwal et al. for an analysis of various loss function choices. For simplicity, we choose the per-token reverse KL — the divergence between the student’s (
π
θ
π 
θ
...
teacher
​
 ) distribution for each token conditioned on the same prior trajectory:
...
Sample trajectories. We sample rollouts from the student exactly as we would in RL. During sampling, RL already computes the student’s logprobs 
log
⁡
π
θ
...
θ
​
 (x) for use as part of the importance-sampling loss.
Compute reward. We query the teacher client with compute_logprobs on the sampled trajectories, which returns the teacher’s logprobs 
log
⁡
π
teacher
...
​
 (x) on the tokens 
x
x sampled by the student.In this post, we do not consider logit (top-k) distillation in any of our experiments, which could be used to further improve compute efficiency. We then use this to calculate the reverse KL.
...
Discussion#
Dense supervision greatly improves compute efficiency#
Reinforcement learning and on-policy distillation both learn via reverse KL, pruning the space of actions present in the base policy. The difference is in the density of reward. In LoRA Without Regret we presented the information-theoretic perspective that reinforcement learning only teaches 
O
(
1
)
O(1) bits per episode. In contrast, distillation teaches 
O
(
N
)
O(N) bits per episode, where 
N
N is the number of tokens. Can we quantify the training efficiency gained through denser reward?
...
We repeat the above experiment of training Qwen3-8B-Base on math, but now with only a single randomly chosen prompt from the dataset.Prompt: “Evaluate the limit: 
lim
⁡
x
→
